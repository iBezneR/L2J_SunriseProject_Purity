    ...
    private void parseDropListItem(Node dropListItem, DropListScope dropListScope, List<IDropItem> drops)
    {
        ...
        switch (dropListItem.getNodeName().toLowerCase())
        {
            case "item":
            {
                ...
                // Removes Herb items from NPC when it is loaded
                if ((itemId == 8600) || (itemId == 8601) || (itemId == 8602))
                {
                    break;
                }

// 		Needs to be fixed so multiple values can be passed from Config
//				int[] removeItems = {9912, 8600, 8601};
//				if (itemId == removeItems)
//				{
//					break;
//				}
