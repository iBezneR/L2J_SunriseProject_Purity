...
import l2r.gameserver.model.items.type.ItemSkillType;
...
public final class DocumentItem extends DocumentBase
{
			...
			else if ("stats".equalsIgnoreCase(n.getNodeName()))
			{
				...
				for (Node b = n.getFirstChild(); b != null; b = b.getNextSibling())
				{
					if ("stat".equalsIgnoreCase(b.getNodeName()))
					{
						...
						if (!b.getAttributes().getNamedItem("type").getNodeValue().equalsIgnoreCase("randomdamage"))
						{
							_currentItem.item.attach(new FuncTemplate(null, null, "add", 0x00, type, value));
						}
						else if (_currentItem.item instanceof L2Weapon)
						{
							((L2Weapon) _currentItem.item)._rndDam = Integer.parseInt(b.getTextContent());
						}
					}
				}
			}
			else if ("skills".equalsIgnoreCase(n.getNodeName()))
			{
				...
				for (Node b = n.getFirstChild(); b != null; b = b.getNextSibling())
				{
					if ("skill".equalsIgnoreCase(b.getNodeName()))
					{
						...
						final int level = Integer.parseInt(b.getAttributes().getNamedItem("level").getNodeValue());
						
						if (b.getAttributes().getNamedItem("type") != null)
						{
							if (ItemSkillType.valueOf(b.getAttributes().getNamedItem("type").getNodeValue()) != ItemSkillType.NORMAL)
							{
								if (ItemSkillType.valueOf(b.getAttributes().getNamedItem("type").getNodeValue()) == ItemSkillType.ON_UNEQUIP)
								{
									_currentItem.item.setUnEquipSkill(id, level);
								}
								
								if (ItemSkillType.valueOf(b.getAttributes().getNamedItem("type").getNodeValue()) == ItemSkillType.ON_ENCHANT)
								{
									_currentItem.item.setEnchant4Skill(id, level);
								}
								continue;
							}
						}
						
						_currentItem.item.addSkill(id, level);
					}
				}
			}
		}
	...
	private void makeItem() throws InvocationTargetException
	{
		...
		try
		{
			...
			_currentItem.item.parseEnchant();
		}
		catch (Exception e)
		{
