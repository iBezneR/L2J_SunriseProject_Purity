...
public abstract class DocumentBase
{
	...
	protected Condition parsePlayerCondition(Node n, Object template)
	{
		...
		for (int i = 0; i < attrs.getLength(); i++)
		{
			...
			if ("hasSkill".equalsIgnoreCase(a.getNodeName()))
			{
				String[] skills = getValue(a.getNodeValue(), null).split(";");
				cond = joinAnd(cond, new ConditionPlayerHasSkill(skills));
			}
	...
	protected Condition parseTargetCondition(Node n, Object template)
	{
		...
		for (int i = 0; i < attrs.getLength(); i++)
		{
			...
			{
				String[] skills = getValue(a.getNodeValue(), null).split(";");
				cond = joinAnd(cond, new ConditionTargetHasSkill(skills));
			}
			switch (a.getNodeName().toLowerCase())
			{
				case "aggro":
