...
public class MacroList implements IRestorable
{
	...
	public void registerMacro(Macro macro)
	{
		...
		switch (ServerTypeConfigs.SERVER_TYPE)
		{
			...
			case FREYA:
			case H5:
				sendUpdate(updateType);
				break;
			case GC:
			case SL:
				_owner.sendPacket(new SendMacroList(0, 1, macro, updateType));
				break;
		}
	}
	...
	public void deleteMacro(int id)
	{
		...
		switch (ServerTypeConfigs.SERVER_TYPE)
		{
			...
			case FREYA:
			case H5:
				sendUpdate(MacroUpdateType.DELETE);
				break;
			case GC:
			case SL:
				_owner.sendPacket(new SendMacroList(0, 1, removed, MacroUpdateType.DELETE));
				break;
		}
	}
	
	public void sendUpdate(MacroUpdateType updateType)
	{
		_revision++;
		...
		synchronized (_macroses)
		{
			...
			{
				_owner.sendPacket(new SendMacroList(_revision, 0, null, updateType));
			}
			else
			{
				for (Macro m : allMacros)
				{
					_owner.sendPacket(new SendMacroList(_revision, allMacros.size(), m, updateType));
				}
			}
		}
	}
