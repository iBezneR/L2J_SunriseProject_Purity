...
import custom.achievements.AchievementPlayer;
import custom.captcha.CaptchaPlayer;
import custom.erengine.ErPlayerBonuses;
import custom.erengine.ErUtils;
import custom.museum.MuseumPlayer;
import custom.vote.VotingPlayer;
...
import l2r.gameserver.model.actor.tasks.player.*;
...
import l2r.gameserver.model.itemcontainer.*;
...
import l2r.gameserver.network.handlers.gc.packets.out.*;
...
public final class L2PcInstance extends L2Playable
{
	...
	public void untransform()
	{
		if (_transformation != null)
		{
			...
			// Removes the effect of Fast Run skill when dismounting
			stopSkillEffects(9206);
	...
	public void increasePvpKills(L2Character target, boolean event)
	{
		if (target instanceof L2PcInstance)
		{
			if (event || AntiFeedManager.getInstance().check(this, target))
			{
				...
				getAchievementPlayer().addAchievementPoints("pvp", 1);
				((L2PcInstance) target).getAchievementPlayer().addAchievementPoints("deaths", 1);
				_spree++;
				if (getAchievementPlayer().getAchievementCount("sprees") < _spree)
				{
					getAchievementPlayer().setAchievementPoints("sprees", _spree);
				}
				if (_spree >= 3)
				{
					getAchievementPlayer().addAchievementPoints("spreesc", 1);
				}
	...
	private static L2PcInstance restore(int objectId)
	{
			...
			PreparedStatement statement = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			player.rewardSkills();
			// Erlandys Captcha Engine
			ErUtils.getInstance().doPlayerRestore(player);
	...
	public synchronized void store(boolean storeActiveEffects)
	{
		...
		ErUtils.getInstance().doPlayerStore(this);
	...
	public void setNoble(boolean val)
	{
		...
		if (val)
		{
			...
			if ((getAchievementPlayer() != null) && !getAchievementPlayer().isAchievementFinished("gain_noble"))
			{
				getAchievementPlayer().setAchievementPoints("gain_noble", 1);
			}
		}
		else
		{
	...
	private synchronized void cleanup()
	{
		...
		ErUtils.getInstance().doPlayerClean(this);
	...
	public final void sendDamageMessage(L2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		if (miss)
		{
			if (target instanceof L2PcInstance)
			{
				...
				{
					case IL:
						sm = SystemMessage.getSystemMessage(SystemMessageId.AVOIDED_C1_ATTACK);
						sm.addCharName(this);
						break;
					case GF:
					case EPILOGUE:
					...
					case GC:
					case SL:
						sm = SystemMessage.getSystemMessage(SystemMessageId.C1_EVADED_C2_ATTACK);
			...
			{
				case IL:
					sm = SystemMessage.getSystemMessage(SystemMessageId.MISSED_TARGET);
					sm.addPcName(this);
					break;
				case GF:
				case EPILOGUE:
				...
				case GC:
				case SL:
					sm = SystemMessage.getSystemMessage(SystemMessageId.C1_ATTACK_WENT_ASTRAY);
					sm.addPcName(this);
					break;
			}
	...
	{
		final ExPCCafePointInfo wnd = new ExPCCafePointInfo(_pcBangPoints, score, add, duble, 24);
		sendPacket(wnd);
	}
	...
	{
		final ExPCCafePointInfo wnd = new ExPCCafePointInfo(_pcBangPoints, 0, false, false, 24);
		sendPacket(wnd);
	}
    ...
    ErPlayerBonuses _epb = null;

    public ErPlayerBonuses getPlayerBonuses()
    {
        return _epb;
    }

    public void setPlayerBonuses(ErPlayerBonuses epb)
    {
        _epb = epb;
    }

    AchievementPlayer _ap = null;
    ...
    public void setAchievementPlayer(AchievementPlayer ap)
    {
        _ap = ap;
    }
    ...
    CaptchaPlayer _cPlayer = null;

    public CaptchaPlayer getCPlayer()
    {
        return _cPlayer;
    }

    public void setCPlayer(CaptchaPlayer cPlayer)
    {
        _cPlayer = cPlayer;
    }
    ...
    VotingPlayer _vp = null;

    public VotingPlayer getVotingPlayer()
    {
        return _vp;
    }

    public void setVotingPlayer(VotingPlayer epb)
    {
        _vp = epb;
    }
