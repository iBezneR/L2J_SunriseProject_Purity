...
public class L2MonsterInstance extends L2Attackable
{
	...
	public boolean doDie(L2Character killer)
	{
		...
		// Erlandys Achievement Engine + Adding Extra Monster Kill Achievements
		if (killer instanceof L2PcInstance)
		{
			if ((getLevel() + 5) >= killer.getActingPlayer().getLevel())
			{
				killer.getActingPlayer().getAchievementPlayer().addAchievementPoints("monster_kills", 1);
				killer.getActingPlayer().getAchievementPlayer().addAchievementPoints("monster_kills_daily", 1);
				if (isChampion())
				{
					killer.getActingPlayer().getAchievementPlayer().addAchievementPoints("champion_kills", 1);
				}
				if (getId() == 18342)
				{
					killer.getActingPlayer().getAchievementPlayer().addAchievementPoints("monster_kills_gremlin", 1);
				}
				if (getId() == 20120)
				{
					killer.getActingPlayer().getAchievementPlayer().addAchievementPoints("monster_kills_wolf", 1);
				}
				if (getId() == 22235)
				{
					killer.getActingPlayer().getAchievementPlayer().addAchievementPoints("monster_kills_warewolf", 1);
				}
			}
		}
		...
		if (killer instanceof L2PcInstance)
		{
			((L2PcInstance) killer).getCPlayer().increaseKill();
		}
