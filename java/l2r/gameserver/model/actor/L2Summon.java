...
import l2r.gameserver.network.handlers.gc.packets.out.ExPetInfo;
			...
			else
			{
				switch (ServerTypeConfigs.SERVER_TYPE)
				{
					case IL:
						if (pcrit || mcrit)
						{
							sendPacket(isServitor() ? SystemMessageId.CRITICAL_HIT_BY_SUMMONED_MOB : SystemMessageId.CRITICAL_HIT_BY_PET);
						}
						
						sm = SystemMessage.getSystemMessage(SystemMessageId.PET_HIT_FOR_S1_DAMAGE);
						sm.addInt(damage);
						break;
					case GF:
					case EPILOGUE:
					...
					case H5:
						if (pcrit || mcrit)
						{
							getOwner().sendPacket(SystemMessage.getSystemMessage(SystemMessageId.C1_HAD_CRITICAL_HIT).addCharName(this));
						}
