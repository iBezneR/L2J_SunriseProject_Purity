...
import l2r.gameserver.model.actor.tasks.character.*;
...
import l2r.gameserver.model.events.impl.character.*;
...
import l2r.gameserver.network.handlers.gc.packets.out.ExTeleportToLocationActivate;
...
public abstract class L2Character extends L2Object implements ISkillsHolder
{
	...
	public final Set<Integer> _abnormalEffects = new CopyOnWriteArraySet<>();
	...
	public final void startAbnormalEffect(boolean update, AbnormalEffect mask)
	{
		...
		VisualEffect grandEffect = VisualEffect.getByName(mask.getName());
		_abnormalEffects.add(grandEffect.getId());
	...
	public final void startAbnormalEffect(int mask)
	{
		...
		AbnormalEffect effect = AbnormalEffect.getById(mask);
		VisualEffect grandEffect = VisualEffect.getByName(effect.getName());
		_abnormalEffects.add(grandEffect.getId());
		updateAbnormalEffect();
	}
	...
	public final void stopAbnormalEffect(boolean update, AbnormalEffect mask)
	{
		...
		VisualEffect grandEffect = VisualEffect.getByName(mask.getName());
		_abnormalEffects.remove(grandEffect.getId());
		if (update)
		{
			updateAbnormalEffect();
		}
