...
public final class EnchantScroll extends AbstractEnchantItem
{
	...
	public EnchantResultType calculateSuccess(L2PcInstance player, L2ItemInstance enchantItem, EnchantSupportItem supportItem)
	{
		...
		double finalChance = Math.min(chance + bonusRate + supportBonusRate, 100);
		
		if (enchantItem.getItem().getType2() == L2Item.TYPE2_WEAPON)
		{
			finalChance = player.getPlayerBonuses().getBonus(ErObjectType.WeaponEnchant, (int) chance);
		}
		else
		{
			finalChance = player.getPlayerBonuses().getBonus(ErObjectType.ArmorEnchant, (int) chance);
		}
