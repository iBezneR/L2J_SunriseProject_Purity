...
public final class L2Armor extends L2Item
{
	...
	@Override
	public void parseEnchant()
	{
		if (getCrystalType() != CrystalType.NONE)
		{
			if (getType1() == L2Item.TYPE1_WEAPON_RING_EARRING_NECKLACE)
			{
				FuncTemplate ft = new FuncTemplate(null, null, "enchant", -1, Stats.valueOfXml("mDef"), 0);
				attach(ft);
				return;
			}
			...
			switch (_type)
			{
				case SHIELD:
				{
					FuncTemplate ft = new FuncTemplate(null, null, "enchant", -1, Stats.valueOfXml("sDef"), 0);
					attach(ft);
					return;
				}
			}
			
			// Rest of the items armor parts and sigils
			FuncTemplate ft = new FuncTemplate(null, null, "enchant", -1, Stats.valueOfXml("pDef"), 0);
			attach(ft);
		}
	}
}
