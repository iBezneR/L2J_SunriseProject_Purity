...
public final class L2Crest implements IIdentifiable
{
	...
	public String getClientPath(L2PcInstance activeChar)
	{
		...
		switch (getType())
		{
			case PLEDGE:
			{
				activeChar.sendPacket(new PledgeCrest(getId(), _data));
			...
			case PLEDGE_LARGE:
			{
				switch (ServerTypeConfigs.SERVER_TYPE)
				{
					...
					case FREYA:
					case H5:
						activeChar.sendPacket(new ExPledgeCrestLarge(getId(), _data, 0, 0));
						break;
					case GC:
					case SL:
						if (_data != null)
						{
							for (int i = 0; i <= 4; i++)
							{
								if (i < 4)
								{
									...
									activeChar.sendPacket(new ExPledgeCrestLarge(_id, fullChunk, 0, i));
								}
								else
								{
									final byte[] lastChunk = new byte[8320];
									System.arraycopy(_data, (14336 * i), lastChunk, 0, 8320);
									activeChar.sendPacket(new ExPledgeCrestLarge(_id, lastChunk, 0, i));
								}
							}
						}
						break;
			...
			case ALLY:
			{
				activeChar.sendPacket(new AllyCrest(getId(), _data));
				path = "Crest.crest_" + Config.SERVER_ID + "_" + getId();
				break;
			}
		}
