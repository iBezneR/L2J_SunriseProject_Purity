...
public class L2Party extends AbstractPlayerGroup
{
	 ...
	 * @param topLvl
	 * @param partyDmg
	 * @param spItemDropCount
	 * @param target
	 */
	public void distributeXpAndSp(long xpReward_pr, int spReward_pr, long xpReward, int spReward, List<L2PcInstance> rewardedMembers, int topLvl, int partyDmg, int spItemDropCount, L2Attackable target)
	{
		...
		spItemDropCount *= (getBaseExpSpBonus(validMembers.size()) * Config.SP_ITEM_DROP_PERCENT_FROM_SP) / validMembers.size();
		...
		for (L2PcInstance member : rewardedMembers)
		{
			...
			if (validMembers.contains(member))
			{
				...
				if (Config.SP_ITEM_DROP_ENABLE && (spItemDropCount > 0))
				{
					member.addItem("SPBonus", Config.SP_ITEM_ID, (int) (spItemDropCount * (member.isPremium() ? Config.SP_ITEM_DROP_BONUS_FOR_PREMIUM : 1)), null, true);
				}
	...
	private final long calculateExpSpPartyCutoff(L2PcInstance player, int topLvl, long addExp, int addSp, boolean vit)
	{
		...
		if (Config.PARTY_XP_CUTOFF_METHOD.equalsIgnoreCase("highfive"))
		{
			...
			for (int[] gap : Config.PARTY_XP_CUTOFF_GAPS)
			{
				if ((lvlDiff >= gap[0]) && (lvlDiff <= gap[1]))
				{
					...
					player.getActingPlayer().getMuseumPlayer().addData("monster_kill_xp", xp);
		...
		{
			player.getActingPlayer().getMuseumPlayer().addData("monster_kill_xp", addExp);
