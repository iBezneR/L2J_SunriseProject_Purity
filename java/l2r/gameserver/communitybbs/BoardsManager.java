...
import custom.achievements.AchievementBBSManager;
import custom.achievements.AchievementsParser;
import custom.classbalancer.ClassBalanceBBSManager;
import custom.museum.MuseumBBSManager;
import custom.museum.MuseumManager;
import custom.skillsbalancer.SkillsBalanceBBSManager;
import custom.vote.VoteBBSManager;
import custom.vote.VoteManager;
...
public class BoardsManager
{
	...
	public void handleCommands(L2GameClient client, String command)
	{
		...
		if (command.startsWith(VoteManager.COMMUNITY_BOARD_TAB) || (!command.startsWith("_bbsvote") && VoteManager.WHOLE_COMMUNITY))
		{
			command = "_bbsvote";
		}
		
		if (command.startsWith("_bbsvote"))
		{
			VoteBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		...
		if (command.startsWith(AchievementsParser.COMMUNITY_BOARD_TAB) || (!command.startsWith("_bbsach") && AchievementsParser.WHOLE_COMMUNITY))
		{
			command = "_bbsach";
		}
		
		if (command.startsWith("_bbsach"))
		{
			AchievementBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}

		command = MuseumManager.getInstance().parsecmd(command);
		...
		else if (command.startsWith("_bbsgetfav") || command.startsWith("bbs_add_fav") || command.startsWith("_bbssearch"))
		{
