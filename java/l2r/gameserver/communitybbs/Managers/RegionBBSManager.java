...
public class RegionBBSManager extends TopBBSManager
{
	...
	public void cbByPass(String command, L2PcInstance activeChar)
	{
		...
		if (command.equals("_bbsreg"))
		{
			String tpl = HtmCache.getInstance().getHtm(activeChar, _pageRegionTpl);
			...
			String html = HtmCache.getInstance().getHtm(activeChar, _pageRegionList);
		...
		else if (command.startsWith("_bbsreglist"))
		{
			...
			String html = HtmCache.getInstance().getHtm(activeChar, _pageRegionSelers);
			...
			String tpl = HtmCache.getInstance().getHtm(activeChar, _pageRegionSTpl);
		...
		else if (command.startsWith("_bbsregview"))
		{
			...
			{
				cbByPass("_bbsreglist;" + townId + ";" + type + ";" + page + ";" + byItem + ";" + search, activeChar);
				return;
			}

			String title = "-";
			String tpl = HtmCache.getInstance().getHtm(seller, _pageRegionStoreTpl);
			...
			if (type < 2)
			{
				...
				for (TradeItem ti : sl.getItems())
				{
					...
					if (item != null)
					{
						...
						stpl = stpl.replace("%item_price%", String.format("%,3d", ti.getPrice()).replace("Â ", ","));
			...
			else
			{
				...
				for (L2ManufactureItem mi : seller.getManufactureItems().values())
				{
					...
					stpl = stpl.replace("%item_price%", String.format("%,3d", mi.getCost()).replace("Â ", ","));
					String desc = "";
					if (item.getCrystalType() != CrystalType.NONE)
					{
			...
			String html = HtmCache.getInstance().getHtm(seller, _pageRegionView);
		...
		else if (command.startsWith("_bbsregtarget"))
		{
			...
			if (seller != null)
			{
				activeChar.sendPacket(new RadarControl(0, 2, seller.getLocation().getX(), seller.getLocation().getY(), seller.getLocation().getZ()));

				if (activeChar.getKnownList().getKnownObject(seller.getObjectId()) != null)
				{
					activeChar.setTarget(seller);
