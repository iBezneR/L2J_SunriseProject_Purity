...
public class RequestGiveNickName extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (activeChar.isNoble() && _target.equalsIgnoreCase(activeChar.getName()))
		{
			...
			{
				activeChar.getAchievementPlayer().setTitle(null);
				if (!AchievementsParser.LEAVE_TITLE_COLOR)
				{
		...
		else
		{
			...
			if (member1 != null)
			{
				...
				if (member != null)
				{
					if (member.getAchievementPlayer().hasTitle() && !AchievementsParser.ALLOW_MEMBER_CHANGE_TITLE && (member != activeChar))
					{
						activeChar.sendMessage("You cannot change title of other player if he has achievement title!");
						return;
					}
					// is target from the same clan?
					member.setTitle(_title);
					if (member.getAchievementPlayer().hasTitle())
					{
						member.getAppearance().setTitleColor(0xFFFF77);
					}
					member.getAchievementPlayer().setTitle(null);
