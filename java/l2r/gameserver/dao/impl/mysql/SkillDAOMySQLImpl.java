...
public class SkillDAOMySQLImpl implements SkillDAO
{
	...
	private static final String SELECT = "SELECT skill_id,skill_level,class_index FROM character_skills WHERE charId=? AND (class_index=? OR class_index=0)";
			...
			PreparedStatement ps = con.prepareStatement(SELECT))
		{
			...
			try (ResultSet rs = ps.executeQuery())
			{
				while (rs.next())
				{
					...
					if ((classIndex != player.getClassIndex()) && !skill.leavesForSubclass())
					{
						continue;
					}
					...
					if (Config.SKILL_CHECK_ENABLE)
					{
						...
						else if (classIndex == player.getClassIndex())
						{
							mustCheck = true;
						}
						else if ((classIndex != player.getClassIndex()) && !skill.leavesForSubclass())
						{
							mustCheck = true;
						}
	...
	public void insert(L2PcInstance player, int classIndex, L2Skill skill)
	{
			...
			PreparedStatement ps = con.prepareStatement(INSERT))
		{
			...
			ps.setInt(4, skill.leavesForSubclass() ? 0 : classIndex);
			ps.execute();
		}
		catch (Exception e)
		{
	...
	public void insert(L2PcInstance player, int newClassIndex, List<L2Skill> newSkills)
	{
			...
			PreparedStatement ps = con.prepareStatement(REPLACE))
		{
			...
			for (final L2Skill addSkill : newSkills)
			{
				...
				ps.setInt(4, addSkill.leavesForSubclass() ? 0 : classIndex);
				ps.addBatch();
			}
			ps.executeBatch();
			con.commit();
	...
	public void update(L2PcInstance player, int classIndex, L2Skill newSkill, L2Skill oldSkill)
	{
			...
			PreparedStatement ps = con.prepareStatement(UPDATE))
		{
			...
			ps.setInt(4, newSkill.leavesForSubclass() ? 0 : classIndex);
			ps.execute();
		}
		catch (Exception e)
		{
