...
public class AuctionHouseManager
{
	...
	public String makeServerAuctionsHtm(L2PcInstance activeChar, int topId, StatsSet set)
	{
		...
		for (int i = currentPage * MAX_ENTRANCES_PER_PAGE; i < ((currentPage * MAX_ENTRANCES_PER_PAGE) + MAX_ENTRANCES_PER_PAGE); i++)
		{
			...
			else
			{
				...
				/**
				 * valores.append("
				 * <td width=250></td>"); valores.append("
				 * <td width=60></td>"); valores.append("
				 * <td width=100></td>"); valores.append("
				 * <td width=130></td>"); valores.append("
				 * <td width=10></td>");
				 */
	...
	public String makePurchaseHtm(L2PcInstance activeChar, int topId, StatsSet set) throws Exception
	{
		...
		if (set.getBoolean("apply", false))
		{
			quantity = "<button value=\"" + DECIMAL_FORMATTER.format(purchaseCount).replace(".", ",") + "\" width=160 height=20 action=\"\" fore=L2UI_CT1.CharacterPassword.CharacterPassword_DF_EditBoxFocus01 back=L2UI_CT1.CharacterPassword.CharacterPassword_DF_EditBoxFocus01>";
		...
		else
		{
			if (!isStackable || (item.getQuantity() == 1))
			{
				quantity = "<button value=\"1\" width=160 height=20 action=\"\" fore=L2UI_CT1.Windows.Windows_DF_Drawer_Bg back=L2UI_CT1.Windows.Windows_DF_Drawer_Bg>";
			}
			else
			{
	...
	public String makeMyAuctionsHtm(L2PcInstance activeChar, StatsSet set)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			if (items.size() > i)
			{
				// Displays Item Icon in the list of Players Auctions. Shows when the line is selected.
		...
		for (int i = startIndex; i < sellList.size(); i++)
		{
			...
			// This is where Icons are displayed when player selects items to sell
			inventario.append("<td fixwidth=42 height=42 valign=top>");
			if (i == selectedItemIndex)
			{
				inventario.append("<table width=42 height=42 cellpadding=2 cellspacing=0 background=L2UI_CT1.AttendCheckWnd.Attend_DayObject_VIPpannel><tr><td align=center valign=middle>");
				inventario.append("<button value=. width=32 height=32 action=\"\" back=" + sellList.get(i).getItem().getIcon() + " fore=" + sellList.get(i).getItem().getIcon() + ">");
			...
			else
			{
				...
				inventario.append("<button value=. width=32 height=32 action=\"bypass " + AuctionBBSManager.getInstance().BBS_COMMAND + ";my;itemId" + sellList.get(i).getObjectId() + ";page" + currentPage + "\" back=" + sellList.get(i).getItem().getIcon() + " fore=" + sellList.get(i).getItem().getIcon() + ">");
				inventario.append("</td></tr></table>");
			}
		...
		{
			flechasInventario.append("<td fixheight=16><button value=. width=16 height=16 action=\"bypass " + AuctionBBSManager.getInstance().BBS_COMMAND + ";my;page0" + (selectedItemId > 0 ? ";itemId" + selectedItemId : "") + "\" fore=L2UI_CH3.joypad_shortcut back=L2UI_CH3.joypad_shortcut></td>");
		}
		flechasInventario.append("</tr>");
		flechasInventario.append("<tr>");
		...
		{
			flechasInventario.append("<td fixheight=16><button value=. width=16 height=16 action=\"bypass " + AuctionBBSManager.getInstance().BBS_COMMAND + ";my;page" + (currentPage - 1) + (selectedItemId > 0 ? ";itemId" + selectedItemId : "") + "\" fore=L2UI_CH3.shortcut_nextv back=L2UI_CH3.shortcut_nextv></td>");
		}
		flechasInventario.append("</tr>");
		flechasInventario.append("<tr>");
		flechasInventario.append("<td></td>");
		...
		{
			flechasInventario.append("<td fixheight=16><button value=. width=16 height=16 action=\"bypass " + AuctionBBSManager.getInstance().BBS_COMMAND + ";my;page" + (currentPage + 1) + (selectedItemId > 0 ? ";itemId" + selectedItemId : "") + "\" fore=L2UI_CH3.shortcut_prevv back=L2UI_CH3.shortcut_prevv></td>");
		}
		flechasInventario.append("</tr>");
		flechasInventario.append("<tr>");
		flechasInventario.append("<td></td>");
		...
		{
			flechasInventario.append("<td fixheight=16><button value=. width=16 height=16 action=\"bypass " + AuctionBBSManager.getInstance().BBS_COMMAND + ";my;page" + maxPage + (selectedItemId > 0 ? ";itemId" + selectedItemId : "") + "\" fore=L2UI_CH3.joypad_shortcut back=L2UI_CH3.joypad_shortcut></td>");
		}
		flechasInventario.append("</tr>");
		...
		if ((selectedItemId > 0) && (set != null) && set.getBoolean("apply", false))
		{
			...
			quantity = "<button value=\"" + DECIMAL_FORMATTER.format(selectedQuantity).replace(".", ",") + "\" width=120 height=20 action=\"\" fore=L2UI_CT1.Windows_DF_Small_Vertical_SizeControl_Bg_Darker back=L2UI_CT1.Windows_DF_Small_Vertical_SizeControl_Bg_Darker>";
			salePrice = "<button value=\"" + DECIMAL_FORMATTER.format(selectedSalePrice).replace(".", ",") + "\" width=120 height=20 action=\"\" fore=L2UI_CT1.Windows_DF_Small_Vertical_SizeControl_Bg_Darker back=L2UI_CT1.Windows_DF_Small_Vertical_SizeControl_Bg_Darker>";
		...
		else if (selectedItemId > 0)
		{
			...
			if (!isStackable || (itemCount == 1))
			{
				...
				quantity = "<button value=\"1\" width=120 height=20 action=\"\" back=L2UI_CT1.Windows_DF_Small_Vertical_SizeControl_Bg fore=L2UI_CT1.Windows_DF_Small_Vertical_SizeControl_Bg>";
			}
			else
			{
		...
		synchronized (_entrancesById)
		{
			...
			for (AuctionHouseEntrance entrance : _entrancesById.values())
			{
				for (AuctionHouseEntranceItem item : entrance.getItems())
				{
					...
					activeChar.sendMessage("Auction cancelled successfully");
	...
	private synchronized void purchaseItem(L2PcInstance activeChar, StatsSet set, AuctionHouseEntranceItem item, int purchaseCount)
	{
		...
		synchronized (_entrancesById)
		{
			...
			activeChar.sendMessage("You have successfully purchased " + newItem.getItem().getName());
		}
	}
