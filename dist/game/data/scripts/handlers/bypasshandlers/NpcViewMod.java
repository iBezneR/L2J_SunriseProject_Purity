...
public class NpcViewMod implements IBypassHandler
{
    ...
    public static void sendNpcView(L2PcInstance activeChar, L2Npc npc)
    {
        ...
        /* Multiplies HP value in 'NpcViewMode' of various kinds of monsters by the corresponding value from configs. Converts double to int. */
        double hpBoostDouble = 1.0;
        int hpBoost;

        if (npc.isChampion())
        {
            hpBoost = (int) (hpBoostDouble * Config.L2JMOD_CHAMPION_HP);
        }
        else if (npc.isRaid())
        {
            hpBoost = (int) (hpBoostDouble * Config.RAID_HP_MULTIPLIER);
        }
        else if (npc.isMinion())
        {
            hpBoost = (int) (hpBoostDouble * Config.MINION_HP_MULTIPLIER);
        }
        else if (npc instanceof L2GrandBossInstance)
        {
            hpBoost = (int) (hpBoostDouble * Config.GRAND_HP_MULTIPLIER);
        }
        else if (npc instanceof L2DefenderInstance)
        {
            hpBoost = (int) (hpBoostDouble * Config.DEFENDER_HP_MULTIPLIER);
        }
        else
        {
            hpBoost = (int) (hpBoostDouble * Config.MONSTER_HP_MULTIPLIER);
        }
        html.setFile(activeChar, activeChar.getHtmlPrefix(), "data/html/mods/NpcView/Info.htm");
        html.replace("%name%", npc.getName());
        html.replace("%hpGauge%", HtmlUtil.getHpGauge(250, (long) npc.getCurrentHp() * hpBoost, npc.getMaxHp() * hpBoost, false)); // TODO: hpBoost should not be rounded but stay as double
    ...
    public static void sendNpcDropList(L2PcInstance activeChar, L2Npc npc, DropListScope dropListScope, int page)
    {
        ...
        for (int i = start; i < end; i++)
        {
            ...
            else if (dropItem instanceof GroupedGeneralDropItem)
            {
                ...
                // If a drop item group have less than 1 item then the group is not displayed
                ...
                else
                {
                    ...
                    sb.append("<table width=332 border=0 cellpadding=2 cellspacing=0 background=\"L2UI_CT1.Windows.Windows_DF_TooltipBG\">");
                    sb.append("<tr><td width=32><table border=0 cellspacing=0 cellpadding=0 width=32 height=32 background=\"icon.action046\">");
                    if ((normalized.getChance() <= 100) || (normalized.getChance() <= 199))
                    {
                        sb.append("<tr><td width=33 height=33 align=center valign=top><img src=\"icon.ensoul_panel_01\" width=32 height=32></td></tr></table>");
                        sb.append("</td><td fixwidth=300 align=center><font name=\"ScreenMessageSmall\" color=\"CD9000\">Group rolled once</font><br1>");
                    }
                    else if ((normalized.getChance() <= 200) || (normalized.getChance() <= 299))
                    {
                        sb.append("<tr><td width=33 height=33 align=center valign=top><img src=\"icon.ensoul_panel_02\" width=32 height=32></td></tr></table>");
                        sb.append("</td><td fixwidth=300 align=center><font name=\"ScreenMessageSmall\" color=\"CD9000\">Group rolled twice</font><br1>");
                    }
                    else
                    {
                        sb.append("<tr><td width=33 height=33 align=center valign=top><img src=\"icon.ensoul_panel_03\" width=32 height=32></td></tr></table>");
                        sb.append("</td><td fixwidth=300 align=center><font name=\"ScreenMessageSmall\" color=\"CD9000\">Group rolled three times</font><br1>");
                    }

                    sb.append("<font color=\"LEVEL\">Chance:     </font>" + chanceFormat.format(Math.min(normalized.getChance(), 100)) + "%");
                    sb.append("</td></tr><tr><td width=32></td><td width=300><table border=0 width=295 cellpadding=0 cellspacing=0><tr>");
                    sb.append("<td width=48 align=right valign=top></td>");
                    ...
                    for (GeneralDropItem generalDropItem : normalized.getItems())
                    {
                        ...
                        sb.append("<tr><td width=52 align=right valign=top><font color=\"C35817\">Amount:</font></td><td width=80 align=center>");
                        ...
                        sb.append("</td><td width=52 align=right valign=top><font color=\"C35817\">Chance:</font></td>");
    ...
    private static void addGeneralDropItem(L2PcInstance activeChar, L2Npc npc, final DecimalFormat amountFormat, final DecimalFormat chanceFormat, final StringBuilder sb, final GeneralDropItem dropItem)
    {
        ...
        sb.append("<tr><td width=45 align=left valign=top><font color=\"C35817\">Amount:</font></td>");
        ...
        sb.append("</td><td width=45 align=left valign=top><font color=\"C35817\">Chance:</font></td>");
        sb.append("<td width=100 align=center>");
